...
class MainWindow(QMainWindow):
    ...
    def export_daily_stats(self):
        """Exportiert tägliche Statistiken für alle Monate als einzelne Sheets"""
        path, _ = QFileDialog.getSaveFileName(
            self,
        ...
        try:
            ...
            options = get_filter_options(session)
            months = sorted([m for m in options.get("months", []) if m])
            if not months:
                QMessageBox.information(self, "Hinweis", "Keine Monate in der Datenbank gefunden.")
                return
            ...
            with pd.ExcelWriter(path, engine="openpyxl") as writer:
                for month in months:
                    ...
                    if daily_df.empty:
                        continue
                    all_months_df.append(daily_df.assign(Monat=month))
                    try:
                        month_num = month.split("-")[1]
                    except Exception:
                        month_num = None
                    month_name = month_names_map.get(month_num, month)
                    # Sheet-Name: z.B. 'Januar_2025'
                    year = month.split("-")[0] if "-" in month else ""
                    sheet_name = f"{month_name}_{year}"[:31]
                    export_with_formatting(
                        writer,
                        daily_df,
                        sheet_name,
                        title=f"PRM Statistik - Tägliche Übersicht ({month_name} {year})",
                        date_range=month,
                    )

                # Gesamtausgabe pro Monat (statt täglich)
                summary_df = compute_monthly_summary_stats(session, in_out, airline, wch_type)
                if not summary_df.empty:
                    # Sheet-Name: 'Gesamt_monatlich'
                    export_with_formatting(
                        writer,
                        summary_df,
                        "Gesamt_monatlich",
                        title="PRM Statistik - Monatsübersicht (Gesamt)",
                        date_range=f"{months[0]} - {months[-1]}" if months else None,
                    )
    ...
    def edit_wch_mapping(self):
        try:
            ...
            QMessageBox.critical(self, "Fehler", f"WCH Mapping konnte nicht geoeffnet werden:\n{e}")
    ...
    def edit_destination_mapping(self):
        try:
            ...
            QMessageBox.critical(self, "Fehler", f"Destination Mapping konnte nicht geoeffnet werden:\n{e}")
