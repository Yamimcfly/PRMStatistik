...
def compute_destination_stats(session, month_key=None, in_out=None, airline=None, wch_category=None) -> pd.DataFrame:
    ...
    categories = ["WCHR", "WCHS", "WCHC", "BLIND", "TAUB", "DPNA", "STRETCHER", "DPAX_NOSHOW"]
    index_cols = ["Destination"]

    # DPNA und MAAS zusammenfassen und als 'DPNA' anzeigen
    df["WCH_Category"] = df["WCH_Category"].replace("MAAS", "DPNA")
...
def compute_quarterly_destination_stats(session, in_out=None, airline=None, wch_category=None) -> pd.DataFrame:
    ...
    if monthly.empty:
        return monthly
    # DPNA und MAAS zusammenfassen
    # Falls noch Spalte 'Total_MAAS' existiert, zu 'Total_DPNA' addieren
    for col in ["Total_MAAS"]:
        if col in monthly.columns:
            monthly["Total_DPNA"] = monthly.get("Total_DPNA", 0) + monthly[col]
